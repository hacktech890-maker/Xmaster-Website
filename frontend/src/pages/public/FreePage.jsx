...
const FreePage = () => {
  ...
  useEffect(() => {
    ...
    setLoadingMore(true);
    fetchVideos(activeTab, page + 1, false);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isNearBottom]);
  ...
  const fetchVideos = async (tab, pg, reset) => {
    ...
    try {
      ...
      if (reset) setVideos(Array.isArray(data) ? data : []);
      else       setVideos((p) => [...p, ...(Array.isArray(data) ? data : [])]);
      setPage(pg);
      ...
      <div className="min-h-screen bg-dark-400">
        <div className="relative bg-gradient-to-b from-dark-500 to-dark-400 border-b border-white/5 py-10 sm:py-12">
          ...
          <div className="container-site relative z-10 text-center">
            ...
            <h1 className="text-3xl sm:text-4xl font-black text-white mb-2 tracking-tight">Free Videos</h1>
            <p className="text-white/50 mb-6 max-w-sm mx-auto">Desi, Indian MMS, amateur and more — all free</p>
            <div className="flex items-center justify-center flex-wrap gap-2">
              {FREE_TABS.map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id)}
                  className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold border transition-all duration-200 ${activeTab === tab.id ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' : 'bg-white/5 text-white/50 border-white/[0.08] hover:bg-white/10 hover:text-white'}`}
                >
                  {tab.icon}{tab.label}
                </button>
              ))}
        ...
        <div className="container-site py-8">
          ...
          {!loading && hasMore && <div ref={infiniteRef} className="h-4 mt-4" />}
