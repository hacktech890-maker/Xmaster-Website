...
// Individual studio/channel page
// Maps studio slug to backend category slug

import React, { useState, useEffect, useRef } from 'react';
import { useParams, Link }   from 'react-router-dom';
import { Helmet }            from 'react-helmet-async';
import {
  FiArrowLeft, FiGrid, FiList,
  FiFilter,
} from 'react-icons/fi';
...
const StudioPage = () => {
  ...
  const fetchVideos = async (pg) => {
    ...
    setError(null);
    try {
      // Try category endpoint first, fallback to tag search
      let res;
      try {
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content={`Watch ${studioName} videos on Xmaster. Premium studio content.`}
        />
      </Helmet>
      ...
      <div className="min-h-screen bg-dark-400">
        ...
        <div
          ...
          style={{ background: studioGradient }}
        >
          {/* Pattern overlay */}
          <div
              ...
              <div>
                ...
                {!loading && (
                  <p className="text-sm text-white/60 mt-1">
                    {total} page{total !== 1 ? 's' : ''} of videos
                  </p>
                )}
              </div>
            </div>
