...
// 'all' = show everything, 'premium' = isPremium:true only, 'public' = isPremium:false only
...
const VideosManager = () => {
  ...
  }, [statusFilter, catFilter, premiumFilter]);
  ...
  const fetchVideos = async (pg = 1) => {
    ...
    try {
      const params = {
        ...
        // Map premiumFilter → backend param
        ...(premiumFilter === 'premium' ? { premium: 'true'  } : {}),
        ...(premiumFilter === 'public'  ? { premium: 'false' } : {}),
      };
  ...
  const handleBulkSetPremium = async (isPremium) => {
    if (selected.size === 0) return;
    const label = isPremium ? 'Premium' : 'Public';
    if (!window.confirm(`Set ${selected.size} video(s) to ${label}?`)) return;
    setProcessing('bulk-premium');
    try {
      await adminAPI.bulkUpdatePremium([...selected], isPremium);
      setVideos((prev) => prev.map((v) => selected.has(v._id) ? { ...v, isPremium } : v));
      setSelected(new Set()); setSelectAll(false);
      toast.success(`${selected.size} video(s) set to ${label}`);
    } catch { toast.error('Bulk premium update failed'); }
    finally  { if (mountedRef.current) setProcessing(null); }
  };
  ...
  const handleTogglePremium = async (video) => {
    setProcessing(video._id);
    try {
      await adminAPI.togglePremium(video._id);
      setVideos((prev) => prev.map((v) => v._id === video._id ? { ...v, isPremium: !v.isPremium } : v));
      toast.success(!video.isPremium ? 'Marked as Premium' : 'Set to Public');
    } catch { toast.error('Failed to update premium status'); }
    finally  { if (mountedRef.current) setProcessing(null); }
  };
      ...
      actions={
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-white/[0.08] text-white/60 border border-white/10 hover:bg-white/15 hover:text-white transition-all duration-200"
          >
      ...
      <div className="space-y-4">
        ...
        <div className="glass-panel rounded-2xl p-4 flex flex-wrap items-center gap-3">
          ...
          {categories.length > 0 && (
            <FilterSelect
              ...
              options={[
                ...
                // Separator-like label trick: show premium categories with ★
                ...categories
                  .filter((c) => c.isPremium)
                  .map((c) => ({ value: c._id, label: `★ ${c.name}` })),
              ]}
          ...
          <button
            ...
            disabled={loading}
            className="p-2.5 rounded-lg text-white/40 hover:text-white hover:bg-white/10 border border-white/[0.08] disabled:opacity-50 transition-all duration-200"
          >
        ...
        {/* ── Bulk action bar ── */}
        {selected.size > 0 && (
          <div className="glass-panel rounded-xl px-4 py-3 flex flex-wrap items-center gap-3 border-primary-600/20 animate-fade-in-down">
            ...
            <div className="flex items-center gap-2 ml-auto flex-wrap">
              ...
              <button
                ...
                disabled={!!processing}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-amber-500/15 text-amber-400 border border-amber-500/20 hover:bg-amber-500/25 disabled:opacity-50 transition-all duration-200"
              >
                <FiStar className="w-3.5 h-3.5" />Set Premium
              </button>
              {/* Bulk set public */}
              <button
                onClick={() => handleBulkSetPremium(false)}
                disabled={!!processing}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-emerald-500/15 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/25 disabled:opacity-50 transition-all duration-200"
              >
                Set Public
              </button>
              {/* Bulk delete */}
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-lg text-xs text-white/40 hover:text-white hover:bg-white/10 transition-all duration-200"
              >
                Clear
              </button>
            </div>
                ...
                <tbody>
                  {videos.map((video) => (
                    <VideoRow
                      ...
                      onTogglePremium={() => handleTogglePremium(video)}
...
const VideoRow = ({
  ...
  onDelete, onStatusToggle, onToggleFeatured, onTogglePremium,
  deleting, processing,
}) => {
  ...
  return (
    <tr
      ref={rowRef}
      className={`transition-colors duration-150 ${selected ? 'bg-primary-600/[0.08]' : ''} ${deleting || processing ? 'opacity-60' : ''}`}
    >
      <td className="pl-4">
      ...
      <td>
        <div className="flex items-center gap-3 py-1">
          <div className="relative w-20 h-12 rounded-lg overflow-hidden flex-shrink-0 bg-dark-300">
            ...
            {/* Premium overlay indicator on thumbnail */}
            {video.isPremium && (
              <div className="absolute top-0.5 left-0.5 flex items-center gap-0.5 bg-amber-500/90 rounded px-1 py-px">
                <FiStar className="w-2 h-2 text-white fill-white" />
              </div>
            )}
          </div>
          <div className="flex-1 min-w-0">
            {editing ? (
              ...
              <p
                className="text-xs font-semibold text-white/80 line-clamp-2 leading-snug cursor-pointer hover:text-white transition-colors duration-150"
                onClick={onStartEdit}
            ...
            <div className="flex items-center gap-2 mt-1">
              {video.featured && (
                <span className="text-[9px] text-amber-400 flex items-center gap-0.5">
                  <FiStar className="w-2.5 h-2.5 fill-amber-400" />Featured
                </span>
              )}
              {video.category && (
      ...
      <td>
        <button
          ...
          disabled={!!processing}
          title={video.isPremium ? 'Click to set Public' : 'Click to set Premium'}
        ...
        >
          {video.isPremium ? '★ Premium' : 'Public'}
        </button>
      </td>
      ...
      <td className="pr-4">
        <div className="flex items-center justify-end gap-1">
          {/* Premium toggle icon button */}
          <button
            onClick={onTogglePremium}
            disabled={!!processing}
            title={video.isPremium ? 'Remove Premium' : 'Make Premium'}
          ...
          <button
            ...
            title="Delete video"
            className="p-1.5 rounded-lg text-white/20 hover:text-red-400 hover:bg-red-500/10 disabled:opacity-50 transition-all duration-150"
          >
            {deleting
...
const TableRowSkeleton = () => (
  <tr className="animate-pulse">
    ...
    <td><div className="h-4 w-16 rounded-full skeleton-shimmer bg-dark-300" /></td>
...
const FilterSelect = ({ value, onChange, options }) => (
  <div className="relative">
    <select
      ...
      className="appearance-none pl-3 pr-8 py-2 rounded-xl text-sm bg-white/[0.06] border border-white/10 text-white/70 hover:border-white/20 focus:border-primary-600/40 focus:outline-none transition-all duration-200 cursor-pointer"
    >
      {options.map((opt) => (
