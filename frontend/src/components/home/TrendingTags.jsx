...
const TrendingTags = ({ title = 'Trending Tags', tags = null, maxTags = 20, showCount = false, compact = false, className = '' }) => {
  ...
  if (loading) return <TagsSkeleton compact={compact} className={className} />;
    ...
    <section className={`space-y-3 ${className}`}>
      {title && (
        <div className="flex items-center gap-3">
          ...
          <h2 className={`font-bold text-white ${compact ? 'text-sm' : 'text-base sm:text-lg'}`}>{title}</h2>
        </div>
      )}
        ...
        {displayTags.map((tag, i) => {
          ...
          return (
            <Link
              ...
              className={`inline-flex items-center gap-1.5 border rounded-full font-medium transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_4px_15px_rgba(0,0,0,0.3)] animate-fade-in ${colorClass} ${isLarge ? compact ? 'px-3 py-1.5 text-xs' : 'px-4 py-2 text-sm' : isMed ? compact ? 'px-2.5 py-1 text-xs' : 'px-3 py-1.5 text-xs' : compact ? 'px-2 py-0.5 text-[10px]' : 'px-2.5 py-1 text-xs'}`}
  ...
  <div className={`space-y-3 ${className}`}>
    ...
    <div className="flex flex-wrap gap-2">
      {[80,65,90,55,70,60,85,50,75,60,70,55].map((w, i) => (
        <div key={i} className={`rounded-full skeleton-shimmer bg-dark-300 ${compact ? 'h-6' : 'h-7'}`} style={{ width: `${w}px` }} />
      ))}
    </div>
  </div>
);
