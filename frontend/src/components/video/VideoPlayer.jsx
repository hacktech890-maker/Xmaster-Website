...
// Premium video player wrapper around abyss.to iframe
// Handles: loading state, error state, responsive sizing
...
const VideoPlayer = ({
  ...
  // Build the embed URL with optional autoplay param
  ...
  if (!embedUrl) {
    return (
      <NoVideoState className={className} />
    );
  }
...
const ClickToPlay = ({ title, embedUrl, onPlay }) => {
  // Try to get a thumbnail for the play screen
  ...
  return (
    ...
    >
      ...
      <div className="
        absolute inset-0
        bg-gradient-to-br from-dark-400 via-dark-400 to-dark-500
      " />
...
const NoVideoState = ({ className }) => (
  <div
    className={`
      relative w-full bg-dark-300 rounded-xl
      flex items-center justify-center
      ${className}
    `}
    style={{ aspectRatio: '16/9' }}
