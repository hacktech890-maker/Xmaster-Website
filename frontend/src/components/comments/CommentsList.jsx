...
const CommentsList = ({ comments = [], loading = false, className = '' }) => {
  if (loading) {
    return (
  ...
  if (!comments || comments.length === 0) {
    return (
      <div className={`py-10 text-center rounded-xl bg-white/[0.02] border border-white/5 ${className}`}>
  ...
  return (
    ...
    ">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-sm text-white/60 leading-relaxed break-words">{comment.message}</p>
      </div>
    </div>
  );
};
